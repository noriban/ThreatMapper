...
from utils.constants import INTEGRATION_TYPE_GOOGLE_CHRONICLE, USER_ROLES, SECRET_TOKEN_LENGTH, \
    ...
    INTEGRATION_TYPE_AWS_SECURITY_HUB, FILTER_TYPE_AWS_ACCOUNT_ID, INTEGRATION_TYPE_AMAZON_SECURITY_LAKE
...
class IntegrationView(MethodView):
        ...
        for integration_type, notifications in response.items():
            ...
            if integration_type == 'google_chronicle':
                for i in range(len(notifications)):
                    if notifications[i]['authorization_key'] != "":
                        notifications[i]['authorization_key'] = mask_api_key(notifications[i]['authorization_key'])
            if integration_type in ['s3', 'aws_security_hub', 'aws_security_lake']:
                for i in range(len(notifications)):
    ...
    def handle_amazon_security_lake_post(self, request_json, user):
        aws_account_id = request_json.get("filters", {}).get("aws_account_id")
        glue_iam_role_arn = request_json.get("glue_iam_role_arn")
        aws_access_key = request_json.get("aws_access_key")
        aws_secret_key = request_json.get("aws_secret_key")
        region_name = request_json.get("region_name")
        if not glue_iam_role_arn:
            raise InvalidUsage("glue_iam_role_arn is required")
        if not aws_account_id:
            raise InvalidUsage("aws_account_id is required")
        elif not aws_access_key:
            raise InvalidUsage("aws_access_key is required")
        ...
        config = json.dumps(
            {"aws_account_id": aws_account_id, "aws_access_key": aws_access_key, "aws_secret_key": aws_secret_key,
             "region_name": region_name, "glue_iam_role_arn": glue_iam_role_arn})
        integration = Integration.query.filter_by(integration_type=INTEGRATION_TYPE_AMAZON_SECURITY_LAKE, config=config
                                                  ).one_or_none()
        if not integration:
            integration = Integration(
                user=user,
            ...
            try:
                ...
                raise InvalidUsage("A similar Amazon Security Lake integration already exists")
        return integration
