...
func (c *appClient) Details() (xfer.Details, error) {
	result := xfer.Details{}
	req, err := c.ProbeConfig.authorizedRequest("GET", c.url("/topology-api"), nil)
	if err != nil {
		return result, err
	}
	...
	if resp.StatusCode != http.StatusOK {
		return result, fmt.Errorf("error response from %s: %s", c.url("/topology-api"), resp.Status)
	}
...
func (c *appClient) pipeConnection(id string, pipe xfer.Pipe) (bool, error) {
	headers := http.Header{}
	c.ProbeConfig.authorizeHeaders(headers)
	url := c.wsURL(fmt.Sprintf("/topology-api/pipe/%s/probe", id))
...
func (c *appClient) PipeClose(id string) error {
	url := c.url(fmt.Sprintf("/topology-api/pipe/%s", id))
